...
/// Generates a sh/bash/zsh preamble using the `${VAR:=value}` idiom, which
/// assigns the default only when VAR is unset. Env vars set before sourcing
/// this script (e.g., `export CHEVRON_TRANSIENT=0` earlier in .zshrc) win.
fn shell_preamble_posix(cfg: &ShellConfig) -> String {
    ...
    let _ = writeln!(
        out,
        ": \"${{CHEVRON_OSC133:={}}}\"",
        bool_to_shell(cfg.osc133)
    );
    let _ = writeln!(
        out,
        ": \"${{CHEVRON_TRANSIENT:={}}}\"",
        bool_to_shell(cfg.transient)
    );
    let _ = writeln!(
        out,
        ": \"${{CHEVRON_TRANSIENT_DURATION_MS:={}}}\"",
        cfg.transient_duration_ms
    );
    let _ = writeln!(
        out,
        ": \"${{CHEVRON_ASYNC:={}}}\"",
        bool_to_shell(cfg.async_render)
    );
    let _ = writeln!(
        out,
        ": \"${{CHEVRON_HISTORY:={}}}\"",
        bool_to_shell(cfg.history)
    );
    let _ = writeln!(out, ": \"${{CHEVRON_LIVE:={}}}\"", bool_to_shell(cfg.live));
    out.push('\n');
    out
}
...
mod tests {
    ...
    fn posix_preamble_defaults_match_today() {
        ...
        assert!(out.contains(": \"${CHEVRON_OSC133:=1}\""));
        assert!(out.contains(": \"${CHEVRON_TRANSIENT:=1}\""));
        assert!(out.contains(": \"${CHEVRON_TRANSIENT_DURATION_MS:=2000}\""));
        assert!(out.contains(": \"${CHEVRON_ASYNC:=0}\""));
        assert!(out.contains(": \"${CHEVRON_HISTORY:=1}\""));
        assert!(out.contains(": \"${CHEVRON_LIVE:=0}\""));
    }
    ...
    fn posix_preamble_reflects_config_overrides() {
        ...
        assert!(out.contains(": \"${CHEVRON_OSC133:=0}\""));
        assert!(out.contains(": \"${CHEVRON_TRANSIENT:=0}\""));
        assert!(out.contains(": \"${CHEVRON_TRANSIENT_DURATION_MS:=500}\""));
        assert!(out.contains(": \"${CHEVRON_ASYNC:=1}\""));
        assert!(out.contains(": \"${CHEVRON_HISTORY:=0}\""));
        assert!(out.contains(": \"${CHEVRON_LIVE:=1}\""));
    }
    ...
    fn init_zsh_with_prepends_preamble_before_body() {
        ...
        // Preamble line appears before the first body marker.
        let preamble_idx = out.find(":=1").expect("preamble missing");
